    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden p-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-slate-900">Welcome to Study Assistant</h1>
          <p className="text-slate-500 text-sm">Your Gateway to Effortless Learning</p>
        </div>

        {/* Tabs */}
        <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
          <button className="flex-1 py-2 text-sm font-semibold bg-blue-600 text-white rounded-lg shadow-sm">Sign Up</button>
          <button onClick={() => navigate('/login')} className="flex-1 py-2 text-sm font-semibold text-slate-500 hover:text-slate-700 transition-all">Sign In</button>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input 
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none"
              />
              <button onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-slate-400 hover:text-slate-600">
          ...
          <div className="bg-slate-50 p-4 rounded-xl space-y-2">
             <p className="text-xs font-semibold text-slate-500 mb-2">Password Strength: <span className={strength.length && strength.numberOrSymbol ? "text-green-600" : "text-orange-500"}>{strength.length && strength.numberOrSymbol ? "Strong" : "Weak"}</span></p>
            <StrengthItem isValid={strength.length} text="At least 8 characters" />
            <StrengthItem isValid={strength.numberOrSymbol} text="Contains a number or symbol" />
          </div>
...
const StrengthItem = ({ isValid, text }) => (
  <div className="flex items-center gap-2 text-xs text-slate-500">
    {isValid ? <Check size={14} className="text-green-500" /> : <div className="w-3.5 h-3.5 rounded-full border border-slate-300"></div>}
    <span className={isValid ? "text-slate-700" : ""}>{text}</span>
  </div>
);

const SocialButton = ({ icon, color }) => (
  <button className={`w-14 h-14 rounded-xl border border-slate-200 flex items-center justify-center text-xl font-bold bg-white hover:bg-slate-50 hover:border-blue-200 transition-all ${color}`}>
    {icon}
  </button>
);
